...
def save_bill():
    ...
    if op>0:
        ...
        f1=open(f"bill{bill_no.get()}.txt","w")
...
def welcome():
    ...
    textarea.insert(END,"\t      Welcome to the shop")
...
title=Label(root,pady=2,text="Shopping Mall",bd=5,bg=bg_color,fg='black',font=('times new roman', 25 ,'bold'),relief=GROOVE,justify=CENTER)
title.pack(fill=X)

#=================Product Frames=================
F1=LabelFrame(root,bd=5,relief=GROOVE,text='Customer Details',font=('times new romon',15,'bold'),fg='black',bg=bg_color)
F1.place(x=0,y=80,relwidth=1)

cname_lbl=Label(F1,text='Customer Name: ',font=('times new romon',18,'bold'),bg=bg_color,fg='black').grid(row=0,column=0,padx=20,pady=5)
cname_txt=Entry(F1,width=15,textvariable=c_name,font='arial 15 bold',relief=SUNKEN,bd=7).grid(row=0,column=1,padx=10,pady=5)

cphone_lbl=Label(F1,text='Phone No. :',font=('times new romon',18,'bold'),bg=bg_color,fg='black').grid(row=0,column=2,padx=20,pady=5)
...
F2 = LabelFrame(root, text='Product Details', font=('times new romon', 18, 'bold'), fg='black',bg=bg_color)
F2.place(x=20, y=180,width=630,height=500)

itm= Label(F2, text='Product Name: ', font=('times new romon',18, 'bold'), bg=bg_color, fg='black').grid(row=0, column=0, padx=30, pady=20)
itm_txt = Entry(F2, width=20,textvariable=item, font='arial 15 bold', relief=SUNKEN, bd=7).grid(row=0, column=1, padx=10,pady=20)

rate= Label(F2, text='Product Rate: ', font=('times new romon',18, 'bold'), bg=bg_color, fg='black').grid(row=1, column=0, padx=30, pady=20)
rate_txt = Entry(F2, width=20,textvariable=Rate, font='arial 15 bold', relief=SUNKEN, bd=7).grid(row=1, column=1, padx=10,pady=20)

n= Label(F2, text='Product Quantity: ', font=('times new romon',18, 'bold'), bg=bg_color, fg='black').grid(row=2, column=0, padx=30, pady=20)
